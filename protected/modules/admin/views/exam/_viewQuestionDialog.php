...
if ($model->question_type == 'DRAG_DROP_TYPEB_ANSWER') {
    $headings = Heading::model()->getHeadingTextforQuestionView($model->question_id);

    $question_parts = QuestionPart::model()->getQuestionPartsforQuestionView($model->question_id);
    ...
    ?>                                       
        ...
        <tbody>
                ...
                for ($i = 0; $i < $ques_part_count; $i++) {
                    echo '<tr>';
                    echo '<td class="span6">' . QuestionPart::model()->getQuestionPartText($question_part_texts_b[$i]) . '</td>';
                    $answers = Answer::model()->getAnswersforQuestionPartid($question_part_texts_b[$i]);
                    foreach ($answers as $answer) {
                ...
                <td  class="span5"><center><button class="bluebtn"><?php echo AnswerText::model()->getAnswerTextById($answer_id[$c]); ?></button></center></td>
            <?php
        }
    ...
    <?php
    $answer_texts_b = AnswerText::model()->getAnswerTextForQuestion($model->question_id);
