    ...
    foreach ($single_answer_data as $single_answer) {
        ...
        if ($single_answer->image_answer == NULL) {
            ...
            if ($single_answer->is_correct == 1) {
                echo '<td><input type="checkbox" id="acheck_' . $count . '" name="correct[' . $count . ']" checked="checked"></td>';
                echo "<td><input type='button' class='greybtn' value='Delete' onclick='RemoveAnswer($single_answer->answer_text_id, $count)' /></td>";
            } else {
                echo '<td><input type="checkbox" id="acheck_' . $count . '" name="correct[' . $count . ']"></td>';
                echo "<td><input type='button' class='greybtn' value='Delete' onclick='RemoveAnswer($single_answer->answer_text_id, $count)' /></td>";
            }
            ...
            echo '<td><input type="hidden" id="imgfile_' . $count . '" name="imageanswer[' . $single_answer->answer_id . ']" value=' . $single_answer->image_answer . ' />' . CHtml::image(Yii::app()->request->baseUrl . '/images/multiple-answer-images/' . $single_answer->image_answer, "", array("width" => "200px", "height" => "72px")) . '</td>';
            if ($single_answer->is_correct == 1) {
                echo '<td><input type="checkbox" id="imgch_' . $count . '" name="correctimg[' . $single_answer->answer_id . ']" checked="checked"></td>';
                echo "<td><input type='button' class='greybtn' value='Delete' onclick='RemoveAnswer($single_answer->answer_id, $count)' /></td>";
            } else {
                echo '<td><input type="checkbox" id="imgch_' . $count . '" name="correctimg[' . $single_answer->answer_id . ']"></td>';
                echo "<td><input type='button' class='greybtn' value='Delete' onclick='RemoveAnswer($single_answer->answer_id, $count)' /></td>";
            }
            $image_count++;
        }
...
if ($image_answer_data != null) {
    ...
    ?>
    <input type="hidden" name="text_answer_edit" value="1">
...
<script type="text/javascript">
    ...
    { 
        $("#removed_answers").append('<input type="hidden" name="deleted_answer[]"  value="' + ans_id +'"/>');
...
<script type="text/javascript">
    ...
    $(document).ready(function(){    
        ...
        $('.add').live('click',function(){
            ...
            $(this).attr('class','del');
            var appendTxt = "<tr><td><textarea id='txt_"+ccount+"' name='newanswer[]' /></td> <td><input type='checkbox' name='newcorrect["+a+"]' id='acheck_"+ccount+"'></td> <td><input type='button' class='add' value='Add More' class='btn'/></td></tr>";
        ...
        $('.addimg').live('click',function(){
            ...
            var appendTxt = "<tr><td><input type='file' id='imgfile_"+ccount+"' name='newimageanswer[]' /></td> <td><input type='checkbox' name='imgcorrect["+k+"]' id='imgch_"+ccount+"'></td> <td><input type='button' class='addimg' value='Add image' class='btn'/></td></tr>";
...
//            }
    ...
    function vaidateTextCheckBox(countCheckBoxValue){
        ...
        $('input[name^="newcorrect"]').each(function() { 
    ...
    function validateImageCheckBox(countCheckBoxValue){
       
        $('input[name^="correctimg"]').each(function() {
        ...
        $('input[name^="imgcorrect"]').each(function() { 
