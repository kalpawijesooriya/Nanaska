...
class Sitting extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('sitting_name', 'required', 'message' => 'Please enter a session name'),
    ...
    public function attributeLabels() {
        return array(
            'sitting_id' => 'Session ID',
            'sitting_name' => 'Session Name',
        );
    }
    ...
    //get sitting name for given sitting_id
    public function getSittingByID($sitting_id) {
        $data = Yii::app()->db->createCommand()
                ->select('sitting_name')
                ->from('sitting')
                ->where('sitting_id=:sitting_id', array(':sitting_id' => $sitting_id))
                ->queryAll();
        return $data[0]['sitting_name'];
    }
