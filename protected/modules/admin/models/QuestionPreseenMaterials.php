...
class QuestionPreseenMaterials extends CActiveRecord {
    ...
    public function getQuestionPreseenMaterials($question_id) {
        $data = Yii::app()->db->createCommand()
                ->select('*')
                ->from('question_preseen_materials')
    ...
    public function getQuestionPreseenMatByTabPosition($question_id, $tab_position) {
        $data = Yii::app()->db->createCommand()
                ->select('*')
                ->from('question_preseen_materials')
                ->where('question_id=:question_id AND preseen_tab_position=:preseen_tab_position', array(':question_id' => $question_id, ':preseen_tab_position' => $tab_position))
                ->queryAll();
