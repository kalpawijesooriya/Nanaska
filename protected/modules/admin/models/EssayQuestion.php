...
class EssayQuestion extends CActiveRecord {
    ...
    public function getEmailEssayDetailsByQuestionId($question_id) {
        ...
        if (isset($data[0]['essay_type'])) {
            return $data[0]['essay_type'];
        } else {
            return $data;
        }
    }

    public function getEssayQuestionDetailsById($question_id) {
        $data = Yii::app()->db->createCommand()
                ->select('*')
                ->from('essay_question')
                ->where('question_id=:question_id', array(':question_id' => $question_id))
    ...
    public function getEssayQuestionDetailsByQId($question_id) {
        $data = Yii::app()->db->createCommand()
                ->select('*')
                ->from('essay_question')
                ->where('question_id=:question_id', array(':question_id' => $question_id))
