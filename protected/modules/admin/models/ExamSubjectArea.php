...
class ExamSubjectArea extends CActiveRecord {
    ...
    public static function getSubjectAreasOfExamById($exam_id) {
        ...
        $subjectAreaData = Array();

        foreach ($data as $d) {
            $examDetails = Exam::model()->getExamDetails($d['exam_id']);

            $subjectAreaDetails = SubjectArea::model()->getSubjectAreaDetails($d['subject_area_id']);

            $subjectAreaData[] = array("exam_details" => $examDetails, "exam_subject_area_details" => $d, "subject_area_details" => $subjectAreaDetails);
        }
        return $subjectAreaData;
    }
